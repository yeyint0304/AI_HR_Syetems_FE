 ...
 */
import { PUT } from "@/app/api/timesheet-periods/[id]/lock/route";
...
describe("PUT /api/timesheet-periods/[id]/lock", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("401s when there is no access-token cookie", async () => {
    ...
    const response = await PUT(new Request("http://localhost/api/timesheet-periods/1/lock"), routeParams("1"));
  ...
  it("locks the period and returns 200 on success", async () => {
    ...
    (backendApiClient.put as jest.Mock).mockResolvedValueOnce({
      data: { StatusCode: 200, IsSuccess: true, Message: "Period locked.", Data: { LockedAt: "2025-02-01T00:00:00Z" } },
    });
    ...
    expect(body.message).toBe("Period locked.");
    expect(backendApiClient.put).toHaveBeenCalledWith(
      "/TimesheetPeriod/LockTimesheetPeriod/1",
      null,
  ...
  it("forwards the backend's message for an expected 4xx logical failure", async () => {
    (getAccessToken as jest.Mock).mockResolvedValueOnce(projectAdminToken);
    (backendApiClient.put as jest.Mock).mockResolvedValueOnce({
      data: { StatusCode: 409, IsSuccess: false, Message: "Period is already locked.", Data: null },
    });
    ...
    expect(body.message).toBe("Period is already locked.");
  });
  ...
  it("sanitizes a 5xx envelope-based logical failure instead of forwarding the raw backend message", async () => {
    ...
    expect(body.message).toBe("Unable to lock the timesheet period.");
  });
