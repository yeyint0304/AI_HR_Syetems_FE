...
describe("POST /api/auth/login", () => {
  ...
  it("resolves username from the Auth/Login response body, since the real backend's JWT carries no username claim", async () => {
    const accessToken = buildToken({
      sub: "1",
    ...
    const response = await POST(
      jsonRequest({ usernameOrEmail: "admin@hrsystem.com", password: "Password1!" })
    );
    const body = await response.json();
