 ...
 */
import { GET } from "@/app/api/reports/roles-summary/export/route";
...
function getRequest(query = ""): Request {
  return new Request(`http://localhost/api/reports/roles-summary/export${query}`);
}
...
describe("GET /api/reports/roles-summary/export", () => {
  ...
  it("calls Report/ExportUserRolesSummary for a SystemAdmin", async () => {
    ...
    expect(backendApiClient.get).toHaveBeenCalledWith("/Report/ExportUserRolesSummary", expect.anything());
  });

  it("calls Report/ExportMyUserRolesSummary for a ProjectAdmin", async () => {
    ...
    expect(response.status).toBe(200);
    expect(backendApiClient.get).toHaveBeenCalledWith("/Report/ExportMyUserRolesSummary", expect.anything());
  });
});
